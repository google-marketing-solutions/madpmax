  ...
  for (const row in values) {
    ...
    if (values[row][inputColumnMap.STATUS] === "") {
      ...
      if (
        ...
        imageType === assetTypes.LANDSCAPE &&
        status == "SUCCESS"
      ) {
...
function getAssetGroupByName(performanceMaxCampaign, assetGroupName) {
  ...
  const assetGroupIterator = performanceMaxCampaign
    .assetGroups()
...
function getImageByName(name) {
  const images = AdsApp.adAssets()
    .assets()
    .withCondition(
      `asset.type = '${assetTypes.IMAGE}' AND ` + `AssetName = '${name}'`
    )
    .get();
  if (images.hasNext()) {
    return images.next();
...
function parseImageUrl(imageUrl) {
  ...
  const imageName =
    imageType +
    "_" +
    imageUrl.substring(imageUrl.lastIndexOf("/") + 1).replace(/\.[^/.]+$/, "");
