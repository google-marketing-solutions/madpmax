...
class AssetGroupService:
  ...
  def __init__(self, google_ads_service, sheet_service, asset_service, google_ads_client):
  ...
  def process_asset_group_data_and_create(
    ...
    for index, asset_group_row in enumerate(asset_group_data):
      if (asset_group_row[newAssetGroupsColumnMap.STATUS] != assetStatus.UPLOADED.value
          and len(asset_group_row) > newAssetGroupsColumnMap.LOGO):
        campaign_alias = self.compile_campaign_alias(asset_group_row)
        campaign_details = self.sheet_service.get_sheet_row(
            campaign_alias, campaign_data, "CAMPAIGN")
        ...
        if campaign_details:
          ...
          customer_name = campaign_details[campaignListColumnMap.CUSTOMER_NAME]
          campaign_id = campaign_details[campaignListColumnMap.CAMPAIGN_ID]
          campaign_name = campaign_details[campaignListColumnMap.CAMPAIGN_NAME]
        ...
        asset_group_status = asset_group_row[
            newAssetGroupsColumnMap.ASSET_GROUP_STATUS]
        asset_group_final_url = asset_group_row[
            newAssetGroupsColumnMap.FINAL_URL]
        asset_group_mobile_url = asset_group_row[
            newAssetGroupsColumnMap.MOBILE_URL]
        asset_group_path1 = asset_group_row[newAssetGroupsColumnMap.PATH1]
        asset_group_path2 = asset_group_row[newAssetGroupsColumnMap.PATH2]
        ...
        operations.append(self.create_asset_group(asset_group_name,
            asset_group_status, asset_group_final_url, asset_group_mobile_url,
            asset_group_path1, asset_group_path2, asset_group_id, campaign_id,
            customer_id))

        # Create 3 mandatory headline assets and extend them to Bulk Operations
        # Object.
        headlines = asset_group_row[
            newAssetGroupsColumnMap.HEADLINE1:
            newAssetGroupsColumnMap.HEADLINE3 + 1]
        headline_resource_names = self.create_mandatory_assets_asset_group(
            headlines, customer_id)
        operations.extend(self.consolidate_mandatory_assets_group_operations(
            headline_resource_names, assetTypes.HEADLINE, asset_group_id,
            customer_id))

        # Create 2 mandatory description assets and extend them to Bulk
        # Operations Object. 
        descriptions = asset_group_row[
            newAssetGroupsColumnMap.DESCRIPTION1:
            newAssetGroupsColumnMap.DESCRIPTION2 + 1]
        description_resource_names = self.create_mandatory_assets_asset_group(
            descriptions, customer_id)
        operations.extend(self.consolidate_mandatory_assets_group_operations(
            description_resource_names, assetTypes.DESCRIPTION, asset_group_id,
            customer_id))
        ...
        other_assets = asset_group_row[
            newAssetGroupsColumnMap.LONG_HEADLINE:
            newAssetGroupsColumnMap.LOGO + 1]
        operations.extend(self.create_other_assets_asset_group(other_assets,
            asset_group_id, customer_id))

        # Send the operations to the Google Ads API and collect the response.
        response, error_message = self.google_ads_service.bulk_mutate_asset_group(
        ...
        if error_message:
          self.sheet_service.variable_update_sheet_status(
              index,
              self.sheet_id,
              newAssetGroupsColumnMap.STATUS,
              assetStatus.ERROR.value,
              error_message,
              newAssetGroupsColumnMap.MESSAGE,
          )
        else:
          self.sheet_service.variable_update_sheet_status(
              index,
          ...
          asset_group_id = response.mutate_operation_responses[
              0].asset_group_result.resource_name.split("/")[-1]
          ...
          self.sheet_service.add_new_asset_group_to_list_sheet(
              [customer_name, customer_id, campaign_name, campaign_id,
                  asset_group_name, asset_group_id])

  def create_other_assets_asset_group(self, assets, asset_group_id,
      customer_id):
    ...
    for index, asset_value in enumerate(assets):
      col_num = newAssetGroupsColumnMap.LONG_HEADLINE + index
      asset_type = newAssetGroupsColumnMap(col_num).name

      asset_operation = (
          self.asset_service.create_asset(asset_type, asset_value, customer_id)
          )
      resource_name = asset_operation.asset_operation.create.resource_name

      if resource_name:
        asset_group_asset_operation = self.asset_service.add_asset_to_asset_group(
                resource_name, asset_group_id, asset_type, customer_id)

        operations.extend([asset_operation,
            asset_group_asset_operation])

    return operations

  def consolidate_mandatory_assets_group_operations(self, resource_names,
        asset_type, asset_group_id, customer_id):
    ...
    for resource_name in resource_names:
      operations.append(self.asset_service.add_asset_to_asset_group(
          resource_name, asset_group_id, asset_type, customer_id))
  ...
  def create_mandatory_assets_asset_group(self, text_assets, customer_id):
    ...
    for text in text_assets:
      operations.append(
        self.asset_service.create_text_asset(text, customer_id))
  ...
  def create_asset_group(self, asset_group_name, asset_group_status,
      asset_group_final_url, asset_group_mobile_url, asset_group_path1,
      asset_group_path2, asset_group_id, campaign_id, customer_id):
    ...
    if not validators.url(asset_group_final_url):
      raise ValueError(f"Final URL '{asset_group_final_url}' is not a valid URL")
    ...
    if not validators.url(asset_group_mobile_url):
      raise ValueError(f"Mobile URL '{asset_group_mobile_url}' is not a valid URL")
    ...
    asset_group.status = self._google_ads_client.enums.AssetGroupStatusEnum[asset_group_status]
