...
class main:
  ...
  def create_api_operations(self):
    ...
    for row in asset_values:
      ...
      if self.assetsColumnMap.ASSET_STATUS < len(row) and row[
          self.assetsColumnMap.ASSET_STATUS
      ...
      asset_group_details = self.sheetService._get_sheet_row(
          row[self.assetsColumnMap.ASSET_GROUP_ALIAS],
          asset_group_values,
          self.assetGroupListColumnMap.ASSET_GROUP_ALIAS.value
      )

      if asset_group_details:
        asset_group_id = asset_group_details[
            self.assetGroupListColumnMap.ASSET_GROUP_ID
        ]
        customer_id = asset_group_details[
            self.assetGroupListColumnMap.CUSTOMER_ID
        ]
      ...
      # Check if Asset Group already exists in Google Ads. If not create Asset Group operation.
        ...
        campaign_details = self.sheetService._get_sheet_row(
            new_asset_group_details[
                self.newAssetGroupsColumnMap.CAMPAIGN_ALIAS
            ],
            campaign_values,
            self.campaignListColumnMap.CAMPAIGN_ALIAS.value,
        )
        ...
        if asset_group_alias not in asset_group_operations:
          ...
          asset_group_sheetlist[asset_group_alias] = [
              asset_group_alias,
              new_asset_group_details[
                  self.newAssetGroupsColumnMap.ASSET_GROUP_NAME
              ],
              asset_group_id
          ] + campaign_details[1:]
      ...
      if asset_type == "YOUTUBE_VIDEO":
        asset_mutate_operation, asset_resource, field_type = (
  ...
  def process_api_operations(
    ...
    self.sheetService.update_asset_sheet_status(
        sheet_results,
        self.sheetService.get_sheet_id(
            self.sheetName, self.googleSpreadSheetId
        ),
        self.googleSpreadSheetId
    )
  ...
  def refresh_spreadsheet(self):
    ...
    self.sheetService.update_campaign_sheet_output(
        results, "CampaignList", self.googleSpreadSheetId
    )
